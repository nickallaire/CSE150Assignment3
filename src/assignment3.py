# -*- coding: utf-8 -*-
"""Contains the classes that represent the XSudoku game state and constraints.
...
#########################################################
## The following are XSudoku-specific classes. ##
#########################################################

class XSudokuVariable(Variable):
    """A Variable that's specific to the XSudoku game (used to keep track of the board location)."""

    def __init__(self, N, i, j, v):
        domain = list(range(1, N + 1)) if v == 0 else [v]
        super(XSudokuVariable, self).__init__('r%dc%d' % (i, j), domain)
        self.i = i
        self.j = j


class XSudoku(object):
    """Contains XSudoku-specific functions."""
    ...
    def __str__(self):
        return XSudoku.print_board(self.N, self.values)
    ...
    def alldiffbox(self, variable_list):
        ...
        and not ((v1.i==v1.j and v2.i==v2.j) or (v1.j == self.N-1-v1.i and v2.j == self.N-1-v2.i))]
    ...
    def to_binary_csp(self):
        ...
        for i, row in enumerate(self.values):
            ...
            for j, v in enumerate(row):
                var=XSudokuVariable(self.N,i,j,v)
                list.append(var)
                loctovar[(i,j)]=var
        ...
        constraints += [constraint for constraint in self.alldiffrowcol([variables[i][i] for i in range(self.N)])]
        constraints += [constraint for constraint in self.alldiffrowcol([variables[i][self.N - 1 - i] for i in range(self.N)])]
